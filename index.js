...
app.put('/api/match/:id', (req, res) => {
    ...
    const { home_team, away_team, home_score, away_score, status, match_date } = req.body;

    const query = `
        UPDATE match
        SET home_team=?, away_team=?, home_score=?, away_score=?, status=?, match_date=?
        WHERE id=?
    `;

    connection.query(
        query,
        [home_team, away_team, home_score, away_score, status, match_date, req.params.id],
        (err, result) => {
            if (err) {
